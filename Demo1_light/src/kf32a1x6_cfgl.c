 ...
 ******************************************************************************
 *  $File Name$       : kf32a1x6_cfgl.c                                       *
 *  $Author$          : ChipON AE/FAE Group                                   *
 *  $Data$            : 2021-07-08                                            *
 *  $Hard Version     : kf32a1x6-MINI-EVB_V1.2                                *
 *  $Description$     : This file provides the Configurable logic unit (CFGL) *
...
/*******************************************************************************
**                     (CFGL) module private definition                       **
*******************************************************************************/
...
RetStatus CFGL1_Clear_RISEINT_Flag (void)
{
    ...
    /*---------------- 清CFGLIFR寄存器CFGL1IF位 ----------------*/
	SFR_CLR_BIT_ASM(CFGL1_SFR->CTL0, CFGL_CTL0_INTP_POS);
	while(((CFGLIFR_SFR->IFR & CFGL_IFR_CFGL1IF)>>CFGL_IFR_CFGL1IF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
	SFR_SET_BIT_ASM(CFGL1_SFR->CTL0, CFGL_CTL0_INTP_POS);
...
RetStatus CFGL2_Clear_RISEINT_Flag (void)
{
    ...
    /*---------------- 清CFGLIFR寄存器CFGL2IF位 ----------------*/
	SFR_CLR_BIT_ASM(CFGL2_SFR->CTL0, CFGL_CTL0_INTP_POS);
	while(((CFGLIFR_SFR->IFR & CFGL_IFR_CFGL2IF)>>CFGL_IFR_CFGL2IF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
	SFR_SET_BIT_ASM(CFGL2_SFR->CTL0, CFGL_CTL0_INTP_POS);
...
/**
  * 描述  清零CFGL中断标志位（清零 CFGL1IF 和 CFGL2IF 位）
  * 输入：无
  * 返回：无
  */
void CFGL_Clear_INT_Flag(void)
{
	CFGLIFR_SFR->IFR = SFR_Config(CFGLIFR_SFR->IFR, ~CFGL_IFR_CFGLIC, 0<<CFGL_IFR_CFGLIC_POS);
}
