...
/**
  ...
  * 输入  SPIx: 指向SPI内存结构的指针，取值为SPI0_SFR~SPI3_SFR。
  *       Select: SPT从动选择位，取值为：
  *                   SPI_PIN_SS0:选择SS0引脚作为从动选择信号,
  *                   SPI_PIN_SS1:选择SS1引脚作为从动选择信号,
  *                   SPI_PIN_SS2:选择SS2引脚作为从动选择信号,
  *                   SPI_PIN_SS3:选择SS3引脚作为从动选择信号。
  * 返回  无。
  */
...
/**
  ...
  * 输入  SPIx: 指向SPI内存结构的指针，取值为SPI0_SFR~SPI3_SFR。
  *       Mode: SPI时钟极性，取值为：
  *                   SPI_SDO_SDI:交叉连接（SDO/SDI） 方式
  *                   SPI_MOSI_MISO:直接连接（MOSI/MISO） 方式。
  * 返回  无。
  */
...
/**
  * 描述  I2S空闲时 MCK 输出模式选择。
  * 输入  SPIx: 指向SPI内存结构的指针，取值为SPI0_SFR~SPI3_SFR。
  *       NewState: TRUE：禁止输出模式， 空闲时 MCK 禁止
  *       			FALSE：连续输出模式， 空闲时 MCK 输出。
  * 返回  无。
  */
