...
# 43 "../src/kf32a1x6_adc.c" 2
# 76 "../src/kf32a1x6_adc.c"
void ADC_Reset (ADC_SFRmap* ADCx)
{
...
void ADC_Cont_Mode_Config (ADC_SFRmap* ADCx, uint32_t ContMode)
{
    ...
    ADCx->CTL0 = SFR_Config(ADCx->CTL0, ~((uint32_t)1<<((18))), ContMode);
}
...
void ADC_Regular_Channel_Config (ADC_SFRmap* ADCx, uint32_t Channel, uint32_t Rank)
{
 ...
 else if (Rank < 9)
 {
  ...
  tmpmask = ((uint32_t)0xFF<<((0))) << (8 * (Rank - 5));
 ...
 else if (Rank < 13)
 {
  ...
  tmpmask = ((uint32_t)0xFF<<((0))) << (8 * (Rank - 9));
 ...
 else
 {
  ...
  tmpmask = ((uint32_t)0xFF<<((0))) << (8 * (Rank - 13));
...
RetStatus ADC_Clear_INT_Flag (ADC_SFRmap* ADCx, uint32_t InterruptType)
{
 ...
 while((((ADCx->STATE)>>tmpreg1) & (uint32_t)0x01) && (wait_flag != (0xFFFF)))
 {
  wait_flag++;
 }
    ADCx->STATE &= ~tmpreg;
