...
/**
  * 描述  配置I2C工作时钟分频比选择。
  * 输入  I2Cx: 指向I2C内存结构的指针，取值为I2C0_SFR~I2C3_SFR。
  *       DIV: I2C_SCK_DIV1 : I2C 工作时钟分频比为 1： 1
  *       	   I2C_SCK_DIV2 : I2C 工作时钟分频比为 1： 2
  *       	   I2C_SCK_DIV4 : I2C 工作时钟分频比为 1： 4
  * 返回  无。
  */
...
/**
  * 描述  配置I2C数据保持时间。
  * 输入  I2Cx: 指向I2C内存结构的指针，取值为I2C0_SFR~I2C3_SFR。
  *       Time: I2C_Keep_Data_3CLK : 3个I2C工作时钟
  *       	   	I2C_Keep_Data_4CLK : 4个I2C工作时钟
  *       	   	I2C_Keep_Data_5CLK : 5个I2C工作时钟
  *       	   	... ...
  *       	   	I2C_Keep_Data_18CLK : 18个I2C工作时钟
  * 返回  无。
  */
...
RetStatus I2C_Clear_Start_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器SIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_SIF)>>I2C_SR_SIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_Stop_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器PIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_PIF)>>I2C_SR_PIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_Ack_Fail_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器AFIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_AFIF)>>I2C_SR_AFIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_Arbitration_Lost_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器ARBLIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_ARBLIF)>>I2C_SR_ARBLIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_SMBus_Alert_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器SMBAIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_SMBAIF)>>I2C_SR_SMBAIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_SMBus_Host_Header_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器SMBHIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_SMBHIF)>>I2C_SR_SMBHIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_SMBus_Device_Default_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器SMBDIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_SMBDIF)>>I2C_SR_SMBDIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
...
RetStatus I2C_Clear_INTERRUPT_Flag (I2C_SFRmap* I2Cx)
{
    ...
    /*---------------- 清零I2C_SR寄存器ISIF位 ----------------*/
    while((((I2Cx->SR) & I2C_SR_ISIF)>>I2C_SR_ISIF_POS)&& (wait_flag != FLAG_TIMEOUT))
    {
