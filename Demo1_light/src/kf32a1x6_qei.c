...
/**
  * 描述  复位QEI外设，使能外设时钟。
  * 输入  QEIx: 指向QEI内存结构的指针，取值为QEI0_SFR。
  * 返回  无。
  */
...
/**
  * 描述  配置低功耗 QEI 工作时钟。
  * 输入  QEIx: 指向QEI内存结构的指针，取值为QEI0_SFR。
  *       QEICLK: 低功耗 QEI 工作时钟，取值范围为:
  *                    QEI_CLK_INTLF: 选择INTLF作为低功耗QEI的工作时钟
  *                    QEI_CLK_LP4M: 选择LP4M作为低功耗QEI的工作时钟
  * 返回  无。
  */
...
RetStatus QEI_Clear_INT_Flag (QEI_SFRmap* QEIx, uint32_t InterruptType)
{
    ...
    QEIx->DIER |= tmpreg;
    while((QEIx->CTL0 & tmpreg1)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
    QEIx->DIER &= ~tmpreg;
...
RetStatus QEI_Clear_Timer_Overflow_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    while((QEIx->CTL0 & QEI_CTL0_TXIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus QEI_Clear_Phase_Error_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    while((QEIx->CTL0 & QEI_CTL0_PHEIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus QEI_Clear_Counter_Error_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    while((QEIx->CTL0 & QEI_CTL0_CNTERRIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
