...
/**
  * 描述  基本定时器(BTIM)配置。
  * 输入  BTIMx: 指向定时器内存结构的指针，取值T14_SFR。
...
/**
  ...
  *       MasterMode: 主模式选择，
  *                   取值范围为：
  *                     BTIM_MASTER_TXUR_SIGNAL: UR位作为触发
  *                     BTIM_MASTER_TXEN_SIGNAL: TXEN作为触发
  *                     BTIM_MASTER_TXIF_SIGNAL: TXIF作为触发
  * 返回  无
  */
