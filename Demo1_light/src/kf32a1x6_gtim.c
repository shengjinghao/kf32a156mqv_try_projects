...
/**
  ...
  *                   GPTIM_T0_INTLF: 低功耗定时器T0选用INTLF时钟
  * 返回  无
  */
...
/**
  ...
  *       MasterMode: 主模式选择，
  *                   取值范围为：
  *                     GPTIM_MASTER_TXUR_SIGNAL: UR位作为触发
  *                     GPTIM_MASTER_TXEN_SIGNAL: TXEN作为触发
...
RetStatus GPTIM_Clear_Overflow_INT_Flag (GPTIM_SFRmap* GPTIMx)
{
    ...
    while(((GPTIMx->CTL1 & GPTIM_CTL1_TXIF)>>GPTIM_CTL1_TXIF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus GPTIM_Clear_Updata_INT_Flag (GPTIM_SFRmap* GPTIMx)
{
    ...
    while(((GPTIMx->CCPXEGIF & CCP_EGIF_TXUIF)>>CCP_EGIF_TXUIF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus GPTIM_Clear_Trigger_INT_Flag (GPTIM_SFRmap* GPTIMx)
{
    ...
    while(((GPTIMx->CCPXEGIF & CCP_EGIF_TXTIF)>>CCP_EGIF_TXTIF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus CCP_Clear_Channel_INT_Flag (CCP_SFRmap* CCPx, uint32_t Channel)
{
    ...
    while(((CCPx->CCPXEGIF & tmpreg)>>(Channel+CCP_EGIF_CC1IF_POS))&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
    CCPx->CCPXSRIC &= ~(CCP_SRIC_CC1IC << Channel);
