 ...
 ******************************************************************************
 *  $File Name$       : kf32a1x6_rtc.c                                 	      *
 *  $Author$          : ChipON AE/FAE Group                                   *
 *  $Data$            : 2021-07-08                                            *
 *  $Hard Version     : kf32a1x6-MINI-EVB_V1.2                                *
 *  $Description$     : This file provides real-time clock (RTC-RRB- related  *
 *  					functions, including:								  *
 *          			+ Real-time clock (RTC) function initialization       *
 *          			  function                                            *
 *          			+ Real-time clock (RTC) control function              *
 *          			+ Real-time clock (RTC) alarm clock configuration     *
 *          			  function                                            *
 *          			+ Real-time clock (RTC) date-time configuration       *
 *          			  function                                            *
 *          			+ Real-time clock (RTC) timer configuration function  *
 *          			+ Real-time clock (RTC) interrupt management function *
 *          			+ Real-time clock (RTC) private function		      *
 ******************************************************************************
...
/*******************************************************************************
**               	(RTC) Module private macro definition                     **
*******************************************************************************/
...
{
    /* 操作RTC模块前，配置相关寄存器 */
    RST_CTL3_Peripheral_Reset_Enable(RST_CTL3_RTCRST, TRUE);
    RST_CTL3_Peripheral_Reset_Enable(RST_CTL3_RTCRST, FALSE);
    PCLK_CTL3_Peripheral_Clock_Enable(PCLK_CTL3_RTCCLKEN, TRUE);
...
void RTC_Configuration (uint32_t TimeFormat, RTC_InitTypeDef * rtcInitStruct)
{
    ...
    CHECK_RESTRICTION(CHECK_RTC_WORK_CLK(rtcInitStruct->m_ClockSource));
    CHECK_RESTRICTION(CHECK_RTC_HOURFORMAT(rtcInitStruct->m_HourFormat));
    if (TimeFormat != RTC_TIME_FORMAT_BCD)
    {
        if (rtcInitStruct->m_HourFormat != RTC_HOUR_FORMAT_24)
        {
...
{
    /* 设置RTC时钟源 */
    rtcInitStruct->m_ClockSource = RTC_WORK_CLK_NO_CLK;
    /* 设置RTC时间格式 */
...
/**
  ...
  * 输入  Source: RTC时钟源选择，取值为：
                    RTC_WORK_CLK_NO_CLK: 无时钟
                    RTC_WORK_CLK_EXTLF: EXTLF作为RTC时钟
                    RTC_WORK_CLK_INTLF: INTLF作为RTC时钟
                    RTC_WORK_CLK_EXTHF_DIV_128: EXTHF经过128分频后作为RTC时钟
  * 返回  无。
  */
void RTC_Work_Clock_Config (uint32_t Source)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_RTC_WORK_CLK(Source));

    /*---------------- 配置BKP_CTL寄存器RTCCLKS位 ----------------*/
    RTC_CR = SFR_Config (RTC_CR, ~RTC_CR_RTCCLKS, Source);
}
...
RetStatus RTC_Clear_Time_Stamp_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_TSIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Time_Stamp_Overflow_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_TSOVFIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Timer1_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_RTCT1IF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Timer0_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_RTCT0IF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Time_Tick_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_TTIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Alarm_B_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_ALRBIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Alarm_A_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_ALRAIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Days_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_DAYIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Hours_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_HOURIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Minutes_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_MINIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus RTC_Clear_Seconds_INT_Flag (void)
{
    ...
    while((RTC_IFR & RTC_IFR_SECIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
