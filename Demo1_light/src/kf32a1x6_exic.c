...
RetStatus EXIC_Clear_Write_Finish_INT_Flag (void)
{
	...
	while(((EXIC_STATE & EXIC_STATE_WENDIF)>>EXIC_STATE_WENDIF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
...
RetStatus EXIC_Clear_Read_Finish_INT_Flag (void)
{
	...
	while(((EXIC_STATE & EXIC_STATE_RENDIF)>>EXIC_STATE_RENDIF_POS)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
