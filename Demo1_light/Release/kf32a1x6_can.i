...
# 30 "D:\\KungFu32\\workspace32\\Demo1_light\\inc/kf32a1x6_can.h" 2
...
static void CAN_RAM_ERASE(void)
{
 ...
 {
  *((uint8_t *)((uint32_t)0x1FFEF800) + i) = 0x00;
 }
}
# 58 "../src/kf32a1x6_can.c"
void CAN_Reset(CAN_SFRmap* CANx)
{
...
uint8_t CAN_Get_Point_Of_RAM_Mailbox(CAN_SFRmap* CANx)
{
    ...
    ((void)0);
    tmpreg = (CANx->RCR & ((uint32_t)0x3F<<((16)))) >> (16);
...
void CAN_Transmit_Message_Configuration (CAN_SFRmap* CANx,CAN_MessageTypeDef* canInitStruct)
{
    ...
    ((void)0);
    ((void)0);
    if(canInitStruct->m_RemoteTransmit == ((uint32_t)1))
    {
     return;
    }else
    {
        ...
        if (canInitStruct->m_FrameFormat != ((uint32_t)1))
        {
            ...
            tmpreg = (canInitStruct->m_Can_ID << (21))
        ...
        else
        {
            ...
            tmpreg = canInitStruct->m_Can_ID << (3);
...
void CAN_Acceptance_FILTER_Config (CAN_SFRmap* CANx, uint8_t FILTER_Group, uint32_t Acceptance)
{
    ...
    *((uint32_t *)(((uint32_t)0x40002900)+FILTER_Group*0x08)) = Acceptance;
}
...
uint32_t CAN_Get_Acceptance_FILTER (CAN_SFRmap* CANx, uint8_t FILTER_Group)
{
    ...
    tmpreg = *((uint32_t *)(((uint32_t)0x40002900)+FILTER_Group*0x08));

    return tmpreg;
}
# 716 "../src/kf32a1x6_can.c"
void CAN_Acceptance_FILTER_Mask_Config (CAN_SFRmap* CANx, uint8_t FILTER_Group, uint32_t Acceptance)
{
    ...
    *((uint32_t *)(((uint32_t)0x40002900)+FILTER_Group*0x08+0x04)) = Acceptance;
}
...
uint32_t CAN_Get_Acceptance_FILTER_Mask (CAN_SFRmap* CANx, uint8_t FILTER_Group)
{
    ...
    tmpreg = *((uint32_t *)(((uint32_t)0x40002900)+FILTER_Group*0x08+0x04));
...
RetStatus CAN_Clear_CANRMC(CAN_SFRmap* CANx)
{
 volatile uint32_t wait_flag = 0x0000;
    ...
    __asm volatile("SET [%0], #%1"::"r"(&(CANx->CTLR)),"i"((15)));
    while((!((CANx->CTLR & ((uint32_t)1<<((15))))>>(15)))&& (wait_flag != (0xFFFF)))
 {
  wait_flag++;
 }
    __asm volatile("CLR [%0], #%1"::"r"(&(CANx->CTLR)),"i"((15)));
 if(wait_flag==(0xFFFF))
 {
  return FAILURE;
 }
...
void CAN_BUSS_OFF_Auto_Reset_Enable(CAN_SFRmap* CANx, FunctionalState NewState)
{
    ...
    CANx->CTLR = SFR_Config(CANx->CTLR, ~((uint32_t)1<<((13))), NewState<<(13));
}
# 794 "../src/kf32a1x6_can.c"
...
RetStatus CAN_Clear_Buffer_Overflow_Flag (CAN_SFRmap* CANx)
{
 volatile uint32_t wait_flag = 0x0000;
    ...
    while((!((CANx->CTLR & ((uint32_t)1<<((16))))>>(16)))&& (wait_flag != (0xFFFF)))
 {
  wait_flag++;
 }
 ...
 wait_flag = 0x0000;
    ...
    while(((CANx->CTLR & ((uint32_t)1<<((17))))>>(17))&& (wait_flag != (0xFFFF)))
 {
  wait_flag++;
 }
...
RetStatus CAN_Transmit_Single (CAN_SFRmap* CANx)
{
 volatile uint32_t wait_flag = 0x0000;
    ...
    while((!(CANx->CTLR & ((uint32_t)1<<((21)))))&& (wait_flag != (0xFFFF)))
 {
  wait_flag++;
 }
    __asm volatile("CLR [%0], #%1"::"r"(&(CANx->CTLR)),"i"((9)));
