 ...
 ******************************************************************************
 *  $File Name$       : kf32a1x6_usart.c                               	      *
 *  $Author$          : ChipON AE/FAE Group                                   *
 *  $Data$            : 2021-07-08                                            *
 *  $Hard Version     : kf32a1x6-MINI-EVB_V1.2                                *
 *  $Description$     : This file provides the functional functions associated*
 *                      with the USART module (Usart) , including:            *
 *          			+ USART module (USART) initialization function        *
 *          			+ USART module (USART) function configuration function*
 *          			+ USART module (USART) 7816 initialization and 		  *
 *          			  configuration functions							  *
 *          			+ USART module (USART) interrupt management function  *
 ******************************************************************************
...
/*******************************************************************************
**                    (USART) Module Private Macro Definitions                **
*******************************************************************************/
...
/**
  * 描述  USART外设复位
  * 输入  USARTx: 指向USART内存结构的指针，取值为USART0_SFR/USART1_SFR/USART2_SFR/USART5_SFR
  * 返回  无。
  */
...
/**
  ...
  * 返回  无
  */
void USART_Configuration (USART_SFRmap* USARTx, USART_InitTypeDef* usartInitStruct)
{
    uint32_t tmpreg;
...
/**
  ...
  * 返回  无
  */
void USART_U7816R_Configuration (USART_SFRmap* USARTx, U7816R_InitTypeDef* usartInitStruct)
{
    uint32_t tmpreg;
...
/**
  ...
  *                   USART_CPOL_HIGH:数据在时钟上升沿同步，
  *                   USART_CPOL_LOW :数据在时钟下降沿同步。
  * 返回  无。
  */
void USART_HalfDuplex_ClockPolarity_Config(USART_SFRmap* USARTx, uint32_t NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
...
/**
  ...
  * 返回  无。
  */
void USART_Address_Detection_Enable(USART_SFRmap* USARTx, FunctionalState NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));
...
/**
  * 描述  设置USART发送间隔字符长度。
  * 输入  USARTx: 指向USART内存结构的指针，取值为USART0_SFR/USART1_SFR/USART2_SFR/USART5_SFR。
  *       UBLength: USART_BLENGTH_13BIT：间隔字符为13个bit长度。
  *       		   USART_BLENGTH_16BIT：间隔字符为16个bit长度。
  * 返回  无。
  */
void USART_Send_Blank_Length_Config(USART_SFRmap* USARTx, uint32_t UBLength)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_USART_BLENGTH(UBLength));

    /*---------------- 设置USART_CTLR寄存器BKPWS位 ----------------*/
    USARTx->CTLR = SFR_Config(USARTx->CTLR, ~USART_CTLR_BKPWS, UBLength);
}
...
/**
  ...
  * 返回  无。
  */
void USART_Transmit_Data_Enable (USART_SFRmap* USARTx, FunctionalState NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * 返回  无。
  */
void USART_Receive_Data_Enable (USART_SFRmap* USARTx, FunctionalState NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * 返回  无。
  */
void USART_Transmit_9Word_Select_Config (USART_SFRmap* USARTx, uint32_t NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
...
/**
  ...
  * 返回  无。
  */
void USART_Infrare_Detector_Enable(USART_SFRmap* USARTx, FunctionalState NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));
...
/**
  ...
  * 返回  无。
  */
void USART_Singlet_Line_Mode_Enable (USART_SFRmap* USARTx, FunctionalState NewState)
{
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));
    CHECK_RESTRICTION(CHECK_FUNCTIONAL_STATE(NewState));
...
{
	uint32_t wait_flag=0x0000,wait_flag1=0x0000;
    /* 参数校验 */
    CHECK_RESTRICTION(CHECK_USART_ALL_PERIPH(USARTx));

    /*---------------- 设置USART_TBUFR寄存器 ----------------*/
    USARTx->TBUFR = Data;
    while(((USARTx->STR & USART_STR_TFEIF))&& (wait_flag1 != FLAG_TIMEOUT))
	{
		wait_flag1++;
	}
    while((!(USARTx->STR & USART_STR_TXEIF))&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
	if((wait_flag==FLAG_TIMEOUT)||(wait_flag1==FLAG_TIMEOUT))
	{
		return FAILURE;
	}
	return SUCCESS;
...
void USART_Receive_Idle_Frame_Config(USART_SFRmap* USARTx, FunctionalState NewState)
{
    ...
    tmpreg = NewState << USART_CTLR_RIDLEIS_POS;
    USARTx->CTLR = SFR_Config(USARTx->CTLR, ~USART_CTLR_RIDLEIS, tmpreg);
}
...
/**
  * 描述  设置DMA读串口接收数据寄存器使能。
  * 输入  USARTx: 指向USART内存结构的指针，取值为USART0_SFR/USART1_SFR/USART2_SFR/USART5_SFR。
  *       NewState: DMA读串口接收数据寄存器使能状态，取值为TRUE 或 FALSE。
  * 返回  无。
  */
...
/**
  * 描述  设置DMA写串口发送数据寄存器使能。
  * 输入  USARTx: 指向USART内存结构的指针，取值为USART0_SFR/USART1_SFR/USART2_SFR/USART5_SFR。
  *       NewState: DMA写串口发送数据寄存器使能状态，取值为TRUE 或 FALSE。
  * 返回  无。
  */
...
FlagStatus USART_Get_Receive_Frame_Idel_Flag(USART_SFRmap* USARTx)
{
    ...
    /*---------------- 读取USART_STR寄存器IDLEIF位 ----------------*/
    if (USARTx->STR & USART_STR_IDLEIF)
    {
        /* USART 接收到空闲帧 */
        return SET;
    }
...
RetStatus USART_Clear_Receive_Overflow_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_OVFEIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Parity_ERROR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_PAREIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Frame_ERROR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_FREIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Blank_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_BRIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Idle_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_IDLEIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Auto_BaudRate_TimeOver_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_ABTOIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_WeakUP_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_WUIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Transmit_ERROR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_TEIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Receive_ERROR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_REIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_CTS_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_CTSIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Receive_BUFR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((USARTx->STR & USART_STR_RDRIF)&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
...
RetStatus USART_Clear_Transmit_BUFR_INT_Flag (USART_SFRmap* USARTx)
{
    ...
    while((!((USARTx->STR & USART_STR_TFEIF)>>USART_STR_TFEIF_POS))&& (wait_flag != FLAG_TIMEOUT))
	{
		wait_flag++;
	}
